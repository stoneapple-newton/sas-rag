...
Use a single whitelisted SAS 9.4 documentation family as the vertical slice. The story should prove the full path from source inventory through parser adapter, normalized document units, chunk emission, provenance validation, embedding, and local Chroma persistence.
...
- Chunks are written to the local Chroma index without duplicate records on rerun.
